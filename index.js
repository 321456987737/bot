...
const CHANNEL_MAIN_ID = process.env.CHANNEL_ID; // Live Stock Bot
const CHANNEL_WEATHER_ID = process.env.CHANNEL_WEATHER_ID; // Weather channel
const CHANNEL_STOCK_PREDICTOR_ID = process.env.CHANNEL_STOCK_PREDICTOR_ID; // Stock Predictor channel
...
function processMessage(m, channelTag) {
  ...
  return {
    ...
    channel: channelTag, // tag to identify which channel this came from
  };
}

async function postToNextBatch(messagesArray, channelTag) {
...
// Common logic to fetch latest 2 and post
async function handleNewMessage(channel, channelTag) {
  try {
    const messagesCol = await channel.messages.fetch({ limit: 2 });
    const msgs = Array.from(messagesCol.values()).reverse().map((m) => processMessage(m, channelTag));

    if (msgs.length > 0) {
      console.log(`📤 [${channelTag}] Sending latest 2 messages`);
      await postToNextBatch(msgs, channelTag);
    }
  } catch (err) {
    console.error(`❌ [${channelTag}] Error fetching or posting messages:`, err);
  }
}
...
client.once("ready", async () => {
  ...
  const channelIds = [
    { id: CHANNEL_MAIN_ID, tag: "LiveStock" },
    { id: CHANNEL_WEATHER_ID, tag: "Weather" },
    { id: CHANNEL_STOCK_PREDICTOR_ID, tag: "StockPredictor" },
  ].filter((c) => c.id);

  for (const { id, tag } of channelIds) {
    try {
      const channel = await client.channels.fetch(id);
      if (!channel) {
        console.warn(`⚠️ [${tag}] Channel not found for ID: ${id}`);
        continue;
      }
      console.log(`✅ [${tag}] Connected to channel: ${channel.name}`);
      await handleNewMessage(channel, tag);
    } catch (err) {
      console.error(`❌ [${tag}] Failed to init channel:`, err);
    }
  }
});

// When any new message appears in any watched channel
client.on("messageCreate", async (message) => {
  const channelMap = {
    [CHANNEL_MAIN_ID]: "LiveStock",
    [CHANNEL_WEATHER_ID]: "Weather",
    [CHANNEL_STOCK_PREDICTOR_ID]: "StockPredictor",
  };

  const tag = channelMap[message.channel?.id];
  if (!tag) return; // ignore other channels

  if (message.author?.bot) return; // ignore bots to prevent loops

  console.log(`💬 [${tag}] New message by ${message.author?.username}`);

  await handleNewMessage(message.channel, tag);
});

// Logging & reconnect handling
